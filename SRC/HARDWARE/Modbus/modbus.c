...
#ifdef DEBUG_MODBUS
                printd("\r\n [%02x]error reply Func:%02x", ModbusPara.sERR, ModbusPara.tBuf[1]);
#endif
        }
        ModbusPara.sERR = ERR_NOT;
}
...
void MB_PresetSingleHoldingRegister(void)
{
        ...
        if ((dvc_addr == ModbusPara.mAddrs) && BURN_ADDR != dvc_addr) {
                ...
                {
                        if ((POS_A == ModbusPara.rBuf[3] || POS_B == ModbusPara.rBuf[3]) && 6 == ModbusPara.rCnt) {
                ...
                {
                        if (SPD_MIN <= ModbusPara.rBuf[3] && SPD_MAX >= ModbusPara.rBuf[3] && 6 == ModbusPara.rCnt) {
                ...
                {
                        if (REPLYMODE_AGS <= ModbusPara.rBuf[3] && REPLYMODE_CUSTOM_3 >= ModbusPara.rBuf[3] &&
                            6 == ModbusPara.rCnt) {
                ...
                {
                        if ((OFF == ModbusPara.rBuf[3] || ON == ModbusPara.rBuf[3]) && 6 == ModbusPara.rCnt) {
                ...
                byteCount++;
                /* 只有当地址不为广播地址且无报错时才回复 */
                if ((MB_Broadcast_ADDR != ModbusPara.tBuf[0]) && (ERR_NOT == ModbusPara.sERR)) {
                        ModbusSend(byteCount); /* 回复 */
                }
#ifdef DEBUG_MODBUS
                printd("\r s:");
