...
void InitValve(void)
{
        ...
        if (valve.status & VALVE_INITING) {
                switch (valve.initStep) {
                        ...
                        case 1: /* 根据光感信号决定移动方向 */
                                if (!MotionStatus[AXSV]) {
                                        ...
                                        //                        ((OPT_BLOCKER == VALVE_OPT) && (valve.portLast !=
                                        //                        POS_A)))
                                        if (OPT_GAP == VALVE_OPT) {
...
#if DIRECTION_SWITCH == 1
                                                        AxisMoveRel(AXSV, (int)ftemp, accel[AXSV], decel[AXSV],
                                                                    speed[AXSV]);
                                                        dbg_printf("\r Half Seal (%d) step%d", valve.initStep,
                                                                   (int)ftemp);
#else
                                                        AxisMoveRel(AXSV, -(int)ftemp, accel[AXSV], decel[AXSV],
                                                                    speed[AXSV]);
                        ...
                        case 7:
                                ...
                                printd("\r POS %c",
                                       (POS_A == valve.portCur
                                            ? 'A'
                                            : (POS_B == valve.portCur ? 'B' : (POS_M == valve.portCur ? 'M' : 'X'))));
                                speed[AXSV] = 100;
                                accel[AXSV] = 100;
                                decel[AXSV] = 200;
                                ...
                                decel[AXSV] *= (rdc.rate);
                                printd("\r\n 恢复电机速度  (%d) spd%d acc%d dec%d", valve.spd, speed[AXSV], accel[AXSV],
                                       decel[AXSV]);
                                break;
                        default:
                                break;
                ...
                if (stepTemp != valve.initStep) {
                        dbg_printf("\r >%d< %d %02x", valve.initStep, valve.bNewInit, valve.status);
        ...
        if ((!(valve.status & VALVE_INITING)) && (!MotionStatus[AXSV])) {
                if (VALVE_RUN_END == valve.status) /* 阀组空闲状态 */
                {
                        if (valve.portCur != valve.portDes && (POS_A == valve.portDes || POS_B == valve.portDes)) {
                                ...
                                dbg_printf("\r\n %s initstep%d (%d) ststus%02x", __FUNCTION__, valve.initStep,
                                           syspara.protectTimeOut, valve.status);
                        }
