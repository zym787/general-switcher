...
- 自动化构建：通过 release_version.py 统一编译、重命名、打包并生成版本日志，支持多目标（O_901/O_906/O_909/A_901/A_906/A_909/B_901/B_906/C_901 系列）。
...
- 版次号：每次发布通过输入版次号（如 r27、r28、r29、r30）进行区分。
...
**更新** 新增从 r27 到 r30 的详细修改说明，涵盖 Modbus 长度校验、地址功能修复、参数范围限制等关键增强
...
- V1.3.1-r28（2026.04.27）
  - 修复 Modbus 复位功能错误，优化 Modbus 性能
- V1.3.1-r29（2026.04.28）
  - 修复 Modbus 地址功能错误，部分 Modbus 参数增加范围限制
- V1.3.1-r30（2026.04.29）
  - 新增 Modbus 长度校验

**章节来源**
- [QHF_v1.3.1修改说明.md:50-171](file://Doc/QHF_v1.3.1修改说明.md#L50-L171)
- [QHF_v1.3.1修改说明.md:177-179](file://Doc/QHF_v1.3.1修改说明.md#L177-L179)
- [release_version.txt:1-289](file://Version/release_version.txt#L1-L289)

### Modbus 协议实现细节

**更新** 新增 Modbus 协议栈的详细实现分析，包括功能码支持、寄存器映射和异常处理机制

- 功能码支持
  - 03H：读保持寄存器（支持单个或多个寄存器读取）
  - 06H：写单个保持寄存器（支持单个寄存器写入）
  - 10H：写多个保持寄存器（批量寄存器写入）
- 寄存器地址映射
  - 状态寄存器：MB_R_STATUS_*（当前通道、控制状态、移动时间等）
  - 运行参数：MB_RW_OPERATE1_*（地址、速度、方向、波特率等）
  - 用户数据：MB_RW_USER_*（序列号等用户可读写数据）
  - 出厂参数：MB_RW_FACTORY*_*（出厂配置参数）
- 异常处理
  - 功能码错误：MB_ERROR_FUNC
  - 地址错误：MB_ERROR_ADDR  
  - 数据错误：MB_ERROR_DATA
  - 设备错误：MB_ERROR_DEVICE

**章节来源**
- [modbus.h:88-198](file://SRC/HARDWARE/modbus/modbus.h#L88-L198)
- [modbus.c:192-366](file://SRC/HARDWARE/modbus/modbus.c#L192-L366)
- [modbus.c:167-186](file://SRC/HARDWARE/modbus/modbus.c#L167-L186)

### 版本管理自动化流程

**更新** 新增版本管理自动化流程的详细说明，包括构建脚本、产物管理和发布流程
